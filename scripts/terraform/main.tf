...
module "ecs-security-group" {
  ...
  secgroup_rules_configuration = [
    ...
    { description      = null, direction = "ingress", ethertype = "IPv4", protocol = "tcp", ports = "16686",
      remote_ip_prefix = "0.0.0.0/0", remote_group_id = null
    },              
...
module "ecs" {
  ...
  instance_image_id  = var.Version == "Jaeger1.68.0-arm-v1" ? local.instance_image_id_maps_v1[data.huaweicloud_availability_zones.az.region] : local.instance_image_id_maps_v1[data.huaweicloud_availability_zones.az.region] 
